...
struct FocusedHistoryNavigationActions: Equatable {
    ...
    // SwiftUI refreshes focused values while rebuilding a reader. Actions have
    // the same meaning for the same controller and availability, even when
    // their closure storage was recreated by that update.
...
struct FocusedHistoryNavigationActionsKey: FocusedValueKey {
    typealias Value = FocusedHistoryNavigationActions
}
...
extension FocusedValues {
    ...
    var historyNavigationActions:
        FocusedHistoryNavigationActionsKey.Value? {
        get { self[FocusedHistoryNavigationActionsKey.self] }
        set {
            self[FocusedHistoryNavigationActionsKey.self] =
                newValue
        }
    }
...
struct NavigationCommands: View {
    ...
    @FocusedValue(\.historyNavigationActions)
    private var focusedHistoryActions

    var body: some View {
        Button(LocalString.common_button_go_back) {
            if let focusedHistoryActions {
                focusedHistoryActions.goBack()
            } else {
                goBack()
            }
        }
            .keyboardShortcut("[")
            .disabled(
                !(focusedHistoryActions?.canGoBack
                    ?? (canGoBack == true))
            )
        Button(LocalString.common_button_go_forward) {
            if let focusedHistoryActions {
                focusedHistoryActions.goForward()
            } else {
                goForward()
            }
        }
            .keyboardShortcut("]")
            .disabled(
                !(focusedHistoryActions?.canGoForward
                    ?? (canGoForward == true))
            )
    }
}
