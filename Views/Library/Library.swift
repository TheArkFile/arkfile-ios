...
struct Library: View {
    ...
    init(
        ...
        let selectedTab = LibraryTabItem.visibleCases.contains(tabItem) ? tabItem : .opened
        self._tabItem = State(initialValue: selectedTab)
        self.categories = categories
    }

    var body: some View {
        TabView(selection: $tabItem) {
            ForEach(LibraryTabItem.visibleCases) { tabItem in
                SheetContent {
                    switch tabItem {
                    case .categories:
