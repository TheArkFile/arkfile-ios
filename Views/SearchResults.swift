...
#if os(iOS)
    @ObservedObject private var arkFileLibrary = ArkFileLocalContentLibrary.shared
#endif
    @FetchRequest(
...
#if os(iOS)
    private var arkFileMatches: ArkFileUnifiedSearchProvider.Matches {
        ArkFileUnifiedSearchProvider.matches(for: viewModel.searchText)
    }
#endif

    private var hasNoSearchableContent: Bool {
#if os(iOS)
        // The bundled catalog, guides and tools remain searchable before any archive is downloaded.
        false
#else
        zimFiles.isEmpty
#endif
...
#if os(iOS)
                    // No ZIM full-text hits, but the query may still match guide
                    // sections, calculators, or library titles.
                    let matches = arkFileMatches
                    if matches.isEmpty {
                        ContentUnavailableView.search(text: viewModel.searchText)
                    } else {
                        ScrollView {
                            ArkFileSearchResultsSection(matches: matches, showsArticlesHeading: false)
                                .padding(.vertical, 8)
                        }
                    }
#else
...
#if os(iOS)
                        ArkFileSearchResultsSection(matches: arkFileMatches)
#endif
...
#if os(iOS)
            Section {
                Text("Search all ArkFile titles, maps, guides, and tools. Article results come from downloaded archives.")
                    .font(.subheadline)
