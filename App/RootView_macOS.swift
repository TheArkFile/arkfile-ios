...
struct RootView: View {
    ...
    @Binding var pendingOpenURLs: PendingWebContentOpenURLs
    ...
    private func drainPendingOpenURLs() {
        var pending = pendingOpenURLs
        let urls = pending.drain()
        pendingOpenURLs = pending
        urls.forEach(handleExternalOpenURL)
    }

    private func handleExternalOpenURL(_ url: URL) {
        /// If the app was just started via URL or file (wasn't open before),
        /// load the content in the first window; otherwise use a new tab in
        ...
        if url.isFileURL {
            ...
            if isAppStart {
                ...
                Task { @MainActor in
                    // Wait one UI cycle for the original window activation,
                    // then create the new tab in the active window.
                    let browser = BrowserViewModel.getCached(tabID: navigation.currentTabId)
    ...
    var body: some View {
        ...
        .task(id: pendingOpenURLs.values) {
