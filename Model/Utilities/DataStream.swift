...
struct DataStream<Element>: AsyncSequence {
    ...
    init?(
        dataProvider: any DataProvider<Element>,
        ranges: [ClosedRange<UInt>],
        managedContentReadToken: ArkFileManagedContentReaderToken? = nil
    ) {
        guard !ranges.isEmpty else { return nil }
        self.iterator = DataStreamIterator<Element>(
...
struct DataStreamIterator<Element>: AsyncIteratorProtocol {
    ...
    /// Explicitly pins the managed-content generation for every provider,
    /// including ordinary LibZim reads that do not own a direct file handle.
    ...
    mutating func next() async throws -> Element? {
        defer { withExtendedLifetime(managedContentReadToken) {} }
        guard managedContentReadToken?.isReleased != true else { return nil }
