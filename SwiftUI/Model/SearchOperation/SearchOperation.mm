    ...
    for (NSUUID *zimFileID in self.zimFileIDs) {
        ...
        try {
            ...
            if (archive.hasFulltextIndex()) {
                [self addIndexSearchResults:archive appZimFileID:zimFileID count:25];
            }
            [self addTitleSearchResults:archive appZimFileID:zimFileID count:25];
...
- (void)addIndexSearchResults:(zim::Archive)archive
                 appZimFileID:(NSUUID *)appZimFileID
                         count:(int)count {
    ...
    try {
        ...
        for (auto result = resultSet.begin(); result != resultSet.end(); result++) {
            ...
            if (path != nil && title != nil) {
                SearchResult *searchResult = [[SearchResult alloc]
                    initWithZimFileID:appZimFileID
                                path:path
                               title:title];
...
/// @param archive to retrieve search results from
/// @param count number of articles to retrieve
- (void)addTitleSearchResults:(zim::Archive)archive
                 appZimFileID:(NSUUID *)appZimFileID
                         count:(int)count {
    if (self.isCancelled) { return; }
    try {
