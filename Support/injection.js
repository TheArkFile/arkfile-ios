function applyArkFileMobileReadingLayout() {
	...
	if (!document.getElementById('arkfile-mobile-reading-layout')) {
		let style = document.createElement('style')
		style.id = 'arkfile-mobile-reading-layout'
		style.textContent = [
			'html, body { max-width: 100% !important; overflow-x: hidden !important; }',
			'body { overflow-wrap: anywhere; }',
			'img, video, canvas, svg { max-width: 100% !important; height: auto !important; }',
			'table, pre { max-width: 100% !important; overflow-x: auto !important; display: block; }'
		].join('\n')
		;(head || document.documentElement).appendChild(style)
	}
}

applyArkFileMobileReadingLayout()
