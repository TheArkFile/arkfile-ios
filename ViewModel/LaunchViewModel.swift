...
enum LaunchSequence: Equatable {
    ...
    var isWebPage: Bool {
        if case .webPage = self {
            return true
        }
        return false
    }

    var showsGlobalSearch: Bool {
        switch self {
        case .webPage:
            return true
        case .catalog(.list):
            return true
        case .loadingData, .catalog(.fetching), .catalog(.welcome):
            return false
        }
    }
}
...
@MainActor protocol LaunchProtocol: ObservableObject {
    ...
    func updateWith(hasZimFiles: Bool, hasSeenCategories: Bool, hasBrowserURL: Bool)
}
...
@MainActor final class NoCatalogLaunchViewModel: LaunchViewModelBase {
    ...
    override func updateWith(hasZimFiles: Bool, hasSeenCategories: Bool, hasBrowserURL: Bool) {
...
@MainActor final class CatalogLaunchViewModel: LaunchViewModelBase {
    ...
    private var hasBrowserURL = PassthroughSubject<Bool, Never>()
    ...
    init(libraryState: Published<LibraryState>.Publisher,
        ...
        hasZIMFiles.combineLatest(
            ...
            browserState: (Bool, LibraryState, Bool?),
            hasSeenCategories: Bool,
            hasBrowserURL: Bool
        ) in
            guard let self = self else { return }
            let (hasZIMs, libState, isBrowserLoading) = browserState

            if hasBrowserURL {
                updateTo(.webPage(isLoading: isBrowserLoading ?? false))
                return
            }

            if !FeatureFlags.hasCatalog {
                switch (isBrowserLoading, hasZIMs) {
                case (.some(true), true):
                    updateTo(.webPage(isLoading: true))
                default:
    ...
    override func updateWith(hasZimFiles: Bool, hasSeenCategories: Bool, hasBrowserURL: Bool) {
        ...
        self.hasBrowserURL.send(hasBrowserURL)
    }
}
